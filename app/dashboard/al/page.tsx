...
export default function ALPage() {
  ...
  useEffect(() => {
    ...
    const subNamesAL = [...new Set(filteredRecords.map(r => r.subjects?.name))];
    const subADataAL = subNamesAL.map(name => filteredRecords.filter(r => r.subjects?.name === name).reduce((sum, r) => sum + (r.a_count || 0), 0));
    const subBDataAL = subNamesAL.map(name => filteredRecords.filter(r => r.subjects?.name === name).reduce((sum, r) => sum + (r.b_count || 0), 0));
    const subCDataAL = subNamesAL.map(name => filteredRecords.filter(r => r.subjects?.name === name).reduce((sum, r) => sum + (r.c_count || 0), 0));
    const subSDataAL = subNamesAL.map(name => filteredRecords.filter(r => r.subjects?.name === name).reduce((sum, r) => sum + (r.s_count || 0), 0));
    const subWDataAL = subNamesAL.map(name => filteredRecords.filter(r => r.subjects?.name === name).reduce((sum, r) => sum + (r.w_count || 0), 0));
    ...
    setChartData({
      ...
      alBar: {
        labels: subNamesAL,
        datasets: [
          { label: 'A', data: subADataAL, backgroundColor: '#059669', borderRadius: 6 },
          { label: 'B', data: subBDataAL, backgroundColor: '#10b981', borderRadius: 6 },
          { label: 'C', data: subCDataAL, backgroundColor: '#3b82f6', borderRadius: 6 },
          { label: 'S', data: subSDataAL, backgroundColor: '#f59e0b', borderRadius: 6 },
          { label: 'F', data: subWDataAL, backgroundColor: '#ef4444', borderRadius: 6 },
        ]
      },
      overallDonut: {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (jsonData.length > 0 && ('Year' in jsonData[0] && 'Subject' in jsonData[0] && 'Total' in jsonData[0])) {
          ...
          throw new Error('Invalid format. Please use columns: Year, Subject, Total, A, B, C, S, F, Absent');
        }
      } catch (err: any) {
        console.error(err);
  ...
  const handleExportPDF = async () => {
    ...
    doc.text("G.C.E. A/L PERFORMANCE REPORT", pageWidth / 2, 28, { align: 'center' });
    ...
    for (let i = 1; i <= pageCount; i++) {
        ...
        doc.text(`${schoolName} | A/L PERFORMANCE REPORT | PAGE ${i} OF ${pageCount}`, pageWidth / 2, 287, { align: 'center' });
    }

    doc.save(`${schoolName.replace(/\s+/g, '_')}_AL_Report_${yearText}_${subText.replace(/\s+/g, '_')}.pdf`);
    } finally {
  ...
  const handleExportExcel = () => {
    ...
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "A-L Results");
    XLSX.writeFile(wb, `A-L-Export-${selectedYear}.xlsx`);
  };
        ...
        <form onSubmit={handleFormSubmit} style={{ display: 'grid', gap: '1.5rem' }}>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '0.5rem' }}>
              ...
              <div>
                <label style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>F</label>
